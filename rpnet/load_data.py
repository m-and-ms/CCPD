...
class labelFpsDataLoader(Dataset):
    def __init__(self, img_dir, imgSize, is_transform=None):
        ...
        for i in range(len(img_dir)):
            self.img_paths += [el for idx,el in enumerate(paths.list_images(img_dir[i]))]
        self.img_paths = self.img_paths[:int(len(self.img_paths)/2)]
    ...
    def __getitem__(self, index):
        ...
        resizedImage = np.transpose(resizedImage, (2,0,1))#BGR -> RBG
        ...
        ori_w, ori_h = [float(int(el)) for el in [img.shape[1], img.shape[0]]]#cv2.shape->h,w,d
        ...
        return resizedImage, new_labels, lbl, img_name,self.total_num
...
class labelTestDataLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return resizedImage,[leftUp, rightDown], lbl, img_name,

#00205459770115-90_85-352&516_448&547-438&539_362&540_360&519_436&518-0_0_22_10_26_29_24-128-7
...
class demoTestDataLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # img = img.astype('float32')
        resizedImage = cv2.resize(img, self.img_size)
        resizedImage = np.transpose(resizedImage, (2,0,1))#GBR->RGB
        resizedImage = resizedImage.astype('float32')
        resizedImage /= 255.0
