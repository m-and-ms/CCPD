...
ap.add_argument("-r", "--resume", default='./model/wR2.pth',
                help="file for re-train")#default=111
...
batchSize = int(args["batchsize"]) if use_gpu else 2#2不会太小吗
    ...
    for epoch in range(epoch_start, num_epochs):
        ...
        for i, (XI, Y, labels, ims) in enumerate(trainloader):
            ...
            if i % 1000 == 1:#each 50个batch写一次日志
                print ('epoch:{}[{}/{}]===>train average loss:{} spend time:{}\n'.format(epoch,i,100000//batchSize, sum(lossAver) / len(lossAver), time()-start))
        ...
        if(epoch%20==0):#每20个epoch存一次模型
            torch.save(model.state_dict(), storeName +'-epoch-'+ str(epoch))#保存模型
